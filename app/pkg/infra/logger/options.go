...
// Options defines the sets of options for App logging
type Options struct {
...
func ApplyOptionsToLoggers(options *Options) error {
	...
	// 	v.Enabled(context.Background(), options.JSONFormatEnabled)
	// }

	appLogLevel := toLogLevel(options.OutputLevel)
	if appLogLevel == UndefinedLevel {
